...
def scan_port(ip, port):
    ...
    s = socket.socket()
    s.settimeout(3) # set timeout if target not responding, you can change the number
...
if mode == "1":
    ...
    for port in range(start_port, end_port + 1):
        t = threading.Thread(target=scan_port, args=(ip, port)) # Threading feature for multiple port
        threads.append(t)
        t.start()
